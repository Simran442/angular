...
@Component({
  selector: 'app-eyespage',
  templateUrl: './eyespage.component.html',
  styleUrls: ['./eyespage.component.css']
})
export class EyespageComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    var items = localStorage.getItem('category');

    $('#' + items + '.side-line').addClass('active');
  ...
  selectshades(item, id, event, product) {
      ...
      res.variants.map(function (img) {
        ...
        if (img.id == id) {
          ...
          res.images.map(function (imgSrc) {
            if (imgSrc.id === img.image_id || imgSrc.id == null) {
              $("img").removeClass('active');
    ...
    for (i = this.index; i <= this.reviewProduct.product.images.length; i++) {
      if (this.reviewProduct.product.images[i] && this.reviewProduct.product.images[i].id == this.productvariant.image_id) {
        this.variantImages = [];

        this.variantImages.push({ src: this.reviewProduct.product.images[i].src })
      } else {
        if (this.reviewProduct.product.images[i] && this.reviewProduct.product.images[i].variant_ids.length == 0) {
          this.variantImages.push({ src: this.reviewProduct.product.images[i].src });
        } else {
          break;
        }
